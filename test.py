...
async def main(comm, host, port, framer=Framer.SOCKET):
    ...
    await csd_mt_94.set_profile_velocity(150000)
    ...
    print(f"Status code: {sw[1]}")
    print(f"Motor code: {cw[1]}")
    
    
    print("------ Move to position ------")
    for i in range(1):
        await csd_mt_94.move_async(0, cs="absolute")
        time.sleep(1)
        await csd_mt_94.move_async(120800, cs="absolute")
    sw = await csd_mt_94.get_status_word()
    cw = await csd_mt_94.get_control_word()
    print(f"Status code: {sw[1]}")
    print(f"Motor code: {cw[1]}")
    ...
    # cw = await csd_mt_94.get_status_word()
    # print(f"Motor code: {cw[1].ready_to_switch_on}")
    ...
    # # Read devuce unformation from modbus little endian address 1152
    # result = await client.read_holding_registers(1152, 2)
    # # print binary representation of the two registers
    # print(f"Register 1: {result.registers[0]:016b}")
    # print(f"Register 2: {result.registers[1]:016b}")
    
    # # Merge the two registers into one 32 bit integer
    # device_info = (result.registers[1] << 16) + result.registers[0]
